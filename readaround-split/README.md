...
This directory holds the **tracked, editable copy** of the kernel source
files this project touches:

```
mm/filemap.c       do_sync_mmap_readahead() / do_async_mmap_readahead()
mm/readahead.c      page_cache_ra_order() and friends
block/bio.c
block/blk-merge.c
block/blk-mq.c
```
...
1. Edit the relevant file(s) in this directory (`mm/*.c`, `block/*.c`).
...
`do_sync_mmap_readahead()` (`mm/filemap.c`), confirming the path fires and
showing the window it computes:
...
이 디렉토리에는 이 프로젝트가 건드리는 커널 소스 파일의
**버전 관리되는, 직접 수정하는 사본**이 들어 있다:

```
mm/filemap.c       do_sync_mmap_readahead() / do_async_mmap_readahead()
mm/readahead.c      page_cache_ra_order() 등
block/bio.c
block/blk-merge.c
block/blk-mq.c
```
...
1. 이 디렉토리(`mm/*.c`, `block/*.c`)에서 해당 파일을 수정한다.
...
현재 `do_sync_mmap_readahead()`(`mm/filemap.c`)의 "mmap read-around"
